...
- **Home window title**: plain "Home," not "Paul Kim's Portfolio" (tried earlier) or "Paul Kim HD" (dropped before that — too reliant on knowing the disk-naming in-joke). The wordmark inside the window already carries the identity/personality role (live Geist Pixel text now, see §5/decision log), so the title bar itself doesn't need to double up on it — it's back to being a plain functional wayfinding label, same as every other window (Work, About, case study titles).
...
- **2026-07-28** — Spacing scale formalized in `styles/tokens.css` (`--space-1` through `--space-9`, 4px–48px) from the margin/padding/gap values already in use sitewide — not a new scale invented on paper. Every hardcoded literal across every component/app CSS file that exactly matched one of those nine values was migrated to reference the token instead; values that didn't exactly match (e.g. Button's own hand-tuned 13px/34px/14px/33px/35px press-shift padding, Tag's 3px/10px, the 70px/46px navbar-clearance offsets, DesktopIcons' 99px icon-column offset) were deliberately left as literals rather than rounded onto the scale. Zero visual change — pure tokenization.
- **2026-07-28** — Typography moved from a two-font (Chicago/Google Sans) to a three-font system: Chicago FLF stays chrome-only (window title bars, navbar, scrollbar labels) — unchanged. Geist Pixel (Square variant — `font-variation-settings: "ELSH" 1`, confirmed against Vercel's `geist-pixel-font` repo) now covers headings (case study titles, section headings, Impressum's h1/h2/h3, the About page name), tags, and CTA/buttons — replacing Chicago in those roles (and the body font, where a couple of headings — case study titles, section headings — had actually been living). Geist (Sans) replaces Google Sans for body/paragraph text; `--font-body` now points at it directly rather than introducing a separate token, so every existing consumer picked it up with no per-file change needed. Both are loaded via the exact Google Fonts embed snippet provided (plain `<link>`, not `next/font/google`), same rationale as the earlier Google Sans embed. Gossip (both densities) is now fully unused and its loaders/tokens were removed. Two chrome gaps were also closed while auditing every `font-family` site: SectionWindow's nested title bar and Window's scrollbar arrow buttons were silently inheriting the body font instead of Chicago (never had an explicit `font-family` at all) — both now explicitly set `var(--font-ui)`, matching the stated "scrollbar labels/window title bars stay Chicago" rule for real. Separately, the Home hero wordmark ("Paul Kim") was converted from a baked bitmap image back to live text in Geist Pixel, matching the About page name's treatment — worth flagging: the bitmap existed specifically because live pixel-style text (Gossip) rendered blurry on Retina displays regardless of font-smoothing settings, tested earlier across Chrome/Safari and multiple density variants; Geist Pixel is a different font but the same general category (pixel-grid display face), so this carries the same risk of recurring and needs visual verification.
